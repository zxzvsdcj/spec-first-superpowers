"""spec-first-superpowers Skill 完整性验证脚本"""
import sys, os, re
...
for rf in ["spec-kit-workflow.md", "openspec-workflow.md", "integration-guide.md"]:
...
for af in ["openspec-constitution.md", "spec-kit-constitution.md"]:
    check(f"assets/constitutions/{af} exists",
...
check("has mode selection logic", "模式选择" in content or "mode" in content.lower())
check("has orchestration flow", "编排" in content or "orchestrat" in content.lower())

# ── 7. 行数控制（精简 Skill 应 ≤ 100 行）─────────────
lines = content.split("\n")
check(f"SKILL.md ≤ 100 lines (actual: {len(lines)})", len(lines) <= 100)

# ── 8. 无冗余内容（这些概念 Claude 已内建）───────────
for pat in ["TDD-First", "RED-GREEN-REFACTOR", "Clean Code", "SOLID", "DRY", "KISS"]:
    check(f'no redundant "{pat}"', pat not in content,
          "should delegate to sub-skills")

# ── 9. 依赖工具可用性 ────────────────────────────────
import shutil
for cmd, label in [("specify", "Spec-Kit CLI"), ("openspec", "OpenSpec CLI")]:
    check(f"{label} ({cmd}) on PATH", shutil.which(cmd) is not None,
          f"run: pip install specify-cli / npm i -g @fission-ai/openspec")

# ── 10. 依赖 Skill 已安装 ────────────────────────────
skills_root = r"C:\Users\Hades\.cursor\skills"
for skill in ["using-superpowers", "planning-with-files", "ui-ux-pro-max"]:
    skill_path = os.path.join(skills_root, skill, "SKILL.md")
    check(f"dependency skill '{skill}' installed",
          os.path.isfile(skill_path), f"not found: {skill_path}")

# ── 报告 ─────────────────────────────────────────────
print("=" * 60)
print("  spec-first-superpowers Skill Validation Report")
print("=" * 60)
for r in results:
    print(r)
print("-" * 60)
